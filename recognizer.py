...
'''
    Run a simple writing canvas for trying out 
    a specific model given by CLI in hdf5 format.
    examples of runs are
...
def progres_bars(results, fill ='█'):
    ''' pretty prints simple horizontal bars for each digit'''
...
def draw_wrapper(modeldir, verbose):
    ''' returns a draw function required used as callback by cv2 
        setting the NN model to be used and the verbose mode'''
   
    committee = [load_model(modeldir+filename) for filename in os.listdir(modeldir) if 'best' in filename]

    def draw(event,x,y,flags,param):
        ''' plots on canvas, resize the canvas to 28x28 and
            apply the model'''
        ...
        if event == cv.EVENT_LBUTTONDOWN:
            ...
            for i in range(512):
             ...
             cv.line(image,point,(x,y),250,35)
             point = (x,y)
